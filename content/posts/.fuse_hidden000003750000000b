...
_publicize_job_id: "43830509559"
author: gmirchev90
categories:
  - code-architecture
date: "2020-05-04T16:21:22+00:00"
guid: http://georgimirchev.com/?p=197
parent_post_id: null
post_id: "197"
summary: |-
  # Why design patterns?
...
tags:
  - design-patterns
timeline_notification: "1588609283"
title: Design Patterns – Books, tutorials &amp; notes
url: /2020/05/04/design-patterns-books-tutorials-notes/
...
- The Gang of Four – Design patterns: Elements of Reusable Object-Oriented Software – [LINK](https://www.goodreads.com/book/show/85009.Design_Patterns)
- Design patterns explained simply – [LINK](https://www.goodreads.com/book/show/21795152-design-patterns-explained-simply)
- Head First: Design Patterns – [LINK](https://www.goodreads.com/book/show/58128.Head_First_Design_Patterns)

Videos:

- YouTube Playlist – [LINK](https://www.youtube.com/playlist?list=PLF206E906175C7E07)

Websites:

- .NET Design Patterns – [LINK](http://www.dofactory.com/net/design-patterns)
- Yegor256 – [ARTICLE](http://www.yegor256.com/2016/02/03/design-patterns-and-anti-patterns.html)

## How to learn?

I learned it by following this algorithm:

1. Split the patterns into categories
1. Fill the patterns in each category
1. Start learning the categories
1. Read a pattern from the Gang of Four book
1. If I don’t get the pattern
   1. Read the same pattern in the Design patterns explained simply book
   1. Watch the video of the pattern in the YouTube playlist
   1. Research the pattern in the .NET Design Patterns and check the real life example
   1. Research the pattern on Google
...
_External iterators_: The idea is to have a common interface called Iterator which has next(), remove() and hasNext() methods. Every class which has data in it can share an iterator so you can use it to traverse the whole structure.

_Internal iterators_: The idea is to not handle the iteration but just write code which is meant to do the thing with each elements. Stream are the type of internal iterators in Java.
...
_Memento_ which is the class that holds the latest state of the object. _Originator_ is the class which creates the memento and passes it’s internal state to it. Also restores from Mementos. _Caretaker_ is the class that is responsible for the memento’s safekeeping. When you want to restore an object’s state you call the Caretaker to get a memento and then pass it to the Originator.
...
This is best explained with example. You have different types of classes. Neccessity, Tobacco, Liquor. You want to tax them differently. You make all of the classes implement a common interface which has a method _tax(Visitor visitor)_ and accept a visitor object. This visitor object will have different _calculateTax()_ methods for the different types of objects it has to calculate the tax for. It looks like a dependency injection of an object into other objects.
